...
struct RenderPhotoReveal : afterhours::System<IsPhotoReveal> {
  virtual void for_each_with(const afterhours::Entity &,
    ...
    if (camera) {
      ...
      vec2 screen_top_left = raylib::GetScreenToWorld2D({0.0f, 0.0f}, camera->camera);
      vec2 screen_bottom_right = raylib::GetScreenToWorld2D({screen_w, screen_h}, camera->camera);
      
      viewport_min_x = std::min(screen_top_left.x, screen_bottom_right.x) - 100.0f;
      viewport_min_y = std::min(screen_top_left.y, screen_bottom_right.y) - 100.0f;
      viewport_max_x = std::max(screen_top_left.x, screen_bottom_right.x) + 100.0f;
      viewport_max_y = std::max(screen_top_left.y, screen_bottom_right.y) + 100.0f;
    }
